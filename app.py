...
# ✅ Use Connection Pooling for Faster MongoDB Queries
uri = "mongodb+srv://Jeevashree:12345@task1.mbwh52m.mongodb.net/?retryWrites=true&w=majority"
client = pymongo.MongoClient(uri, serverSelectionTimeoutMS=5000)  # 5-second timeout
db = client["task1"]
...
# ✅ Home Route to Prevent 404 Errors on Deployment
@app.route("/")
def home():
...
def login_user():
    data = request.get_json()
    user = db.users.find_one({"email": data['email']}, {"password": 1, "_id": 1})  # ✅ Fetch only required fields
